...
# Ensure vLLM service gets the correct model name
# Use VLLM_MODEL_NAME if set, otherwise fall back to MODEL_NAME
export MODEL_NAME=${VLLM_MODEL_NAME:-${MODEL_NAME:-"openai/gpt-oss-20b"}}
...
echo ">>> [vLLM] Using model: $MODEL_NAME"
VLLM_CMD+=" --host $HOST"
VLLM_CMD+=" --port $PORT"
